...
void main(void)
{
	...
	P1IES = BIT1;	//reaguj na zostupnu hranu iba na P1.1
	//P1IES  &= ~ BIT3;	//ak chcem nabeznu hranu....
	P1IFG = 0;		//nuluj vsetkych osem priznakov

	P1IE = BIT1;	//povol spustenie ISR len od pinu P1.1
 ...
 * 2)Okrem standardneho nastavenia pinou na vystup a "presetu" LEDiek sme tentokrat aj povolili rezistory na dafinovanie log. urovne
 * 3)Riadiacie registre sme nastavili tak aby reagovali na zostupnu hranu a taktiez sme prednastavili local a global "allow" pre interupt
 * 4)Vynulovanim flagu na konci po dokonceni blikania lednky sme zamedzili opatovnemu zapinaniu loop-u
 * 5)Nastavenim LMP0_bits sme uviedli CPU do nizkoprikonoveho rezimu
 * 6)Na zaver sme LEDky ovladali enkoderom (jeho otacanim)
 */
