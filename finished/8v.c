...
uint16_t adcRaw = 0; //vyuzivame stdint.h
uint16_t voltage = 0;
...
__interrupt void ADC10_ISR(void)
{
    ...
    //voltage = (float)adcRaw * 3.226;

    //voltage = ((uint32_t)adcRaw * ADC_REF_VOLTAGE) / 1023;
    voltage = ((uint32_t) adcRaw * ADC_REF_VOLTAGE) >> 10; //Hlavny vypocet s right shiftom pre display

    /************** prevod bin do BCD ******************/
    //pomoc=vysledok;
    pomoc = (int) voltage;	//pri odcitavani moze byt "pomoc" aj zaporna
...
__interrupt void dalsie_cislicovky(void)
{
    ...
    case 3:
        UCB0TXBUF = ~(tvary_cislic[tis] | 0x80); //pridali sme ciarku
...
/*
 * ZAVER:
 * 1)Prestudovali sme sposob pripojenia potenciometra
 * 2)Programovali sme v jazyku c
 * 3)Zmenili sme program tak aby nam ukazoval napetie namiesto raw hodnoty
 * 4)Zmenili sme vyocty v programe tak aby nemusel pracovat s float-mi
