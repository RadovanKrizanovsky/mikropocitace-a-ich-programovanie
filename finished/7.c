...
const unsigned char tvary_cislic[10] = { 0x3F, 6, 0x5b, 0x4f, 0x66, 0x6d, 0x7d,
                                         0x07, 0x7F, 0x6F };
// ulozene do FLASH
...
__interrupt void nieco(void)
{
    ...
    if (P1IFG & BIT3)
    {
        if (P1IN & BIT4)
        {
            ...
            jed_sek++;
            // Kontrolujeme ci hodnota nepresahuje
            if (jed_sek == 10)
            {
                jed_sek = 0, des_sek++;
            }
        ...
        //hned aj odosleme a nastavime default hodnotu
        UCB0TXBUF = ~(tvary_cislic[jed_sek]);
        i = 1;
...
/*
 * ZAVER:
 * 1)Prestudovali sme sposob zapojenia 4-cislicovkoveho LED displeja
 * 2)Programovali sme v jazyku c
 * 3)Oboznamili sme sa s IRS spustanou periferiou casovaca
 * 4)Zistili sme ako sa riadi obvod 74HC595 pomocou periferie seriovÈho portu USCI
 * 5)Opravili sme cislicu 6 tak aby bol "vyplneni" aj horny segment
 * 5)Do ISR bol doplneni blok dekrementacie casovych hodnot
 * 5)Na zaver sme do programu implementovali stopky a ich chod riadenia s roznymi funkciami
 */
