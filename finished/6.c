...
__interrupt void rovnost(void)
{
    ...
    switch (direction) {
        case DirLeft:
            if (outLED == 0xFF) { // left end
                outLED = 0x7F; // vrat na predposlednu poziciu
                direction = DirRight; //zmena smeru L to R
                break;
            }
            outLED |= outLED<<1; //postupne rozsvecuje jednu LEDku za druhou (ORuje stale s o jeden kladny bit vacssim cislom)
        break;

        case DirRight:
            if (outLED == 0) { // right end
                outLED = 1; // vrat sa na spat na prvu poziciu
                direction = DirLeft; //zmena smeru R to L
                break;
            }
            outLED &= outLED>>1; //postupne LEDky zhasina smerom k zaciatku
        break;
    }
    ...
    CCR0 += 32000; // dlzka pauzy medzi dvoma vysielaniami. Ako casto sa meni stav LED stlpca
               //Zmencovali sme az kym nestihlo odoslat skor nez chcelo odoslat dalsiu spravu
...
/*
 * ZAVER:
 * 1)Oboznamili sme sa so spôsobom zapojenia rozširujúcej dosky è. 1 (evb1) v priloženej schéme
 * 2)Preštudovali sme spôsob riadenia obvodu 74HC595D a príslušných LED diód a riadenie èinnosti modulu sériového portu USCI v móde SPI.
 * 3)Zmensovali sme CCR0 az kym sa nestihli odoslat predosle data pred zaciatkom odoslania novych
 * 4)Na osciloskope sme pozorovali zmenu datoveho sygnalu
 * 5)Na zaver sme upravili switch tak aby sme vytvorili LEDkovy bargraf
 */
