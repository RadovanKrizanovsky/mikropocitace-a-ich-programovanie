...
void main(void)
{
    ...
    P1IE = BIT1|BIT2; //povolime prerusenia
    P1IFG = 0; //vynulujeme flag
    ...
    TACCR0 = 3000;					// komparacny register CCR0
                                    //startovacia hodnota, od nej frekvencia
                                    // blikania LED nezavisi.

    BCSCTL2 |= SELS;

    TACTL = TASSEL_2 + MC_2; // skontrolovat parametre podla zadania, skomentovat v zavere (zmenili sme na TASSEL_2)
    ...
    TACCTL0 = CCIFG;				//nastavenie priznaku priznaku od komparatora 0
    TACCTL0 = CCIE;                 // povolenie prerusenia od priznaku nastatia rovnosti obsahu TAR a CCR0
    TACCTL1 = CCIE; //povolime flag aj pre cerevenu LED
...
#pragma vector = TIMER0_A0_VECTOR		//skontrolujte nazov vektora => zmenili sme TIMER_A0_VECTOR na TIMER0_A0_VECTOR
__interrupt void komp0(void)
{

    P1OUT ^= 0x01;		// zmen log. stav na P1.0

    TACCR0 += 16384;   //pol sekundu ON pol sekundy OFF
                                    // pripocitaj hodnotu delta k obsahu CCR0
    TACCTL0 &= ~CCIFG;  //vymazeme flag
                                    // frekvencia zavisi od hodnoty delta
}

// Druha obsluzna rutina
#pragma vector = TIMER0_A1_VECTOR       //skontrolujte nazov vektora => zmenili sme TIMER_A0_VECTOR na TIMER0_A0_VECTOR
__interrupt void komp1(void)
{

    P1OUT ^= 0x40;      // zmen log. stav na P1.0

    TACCR1 += 22938;   //tentokrat 1.4 sekundy
                                    // pripocitaj hodnotu delta k obsahu CCR0
    TACCTL1 &= ~CCIFG;  //vymazeme flag
                                    // frekvencia zavisi od hodnoty delta
}
 ...
 * 2)Oboznamili sme sa so schemou casovaca a makrami
 * 3)Po nastaveni povodnych nastaveno sme uspali porcesor
 * 4)V obsluznej rutine ktora blika LEDkou sme zmenili zly nazova vektora
 * 5)Menili sme nastavenia frekvencie, pripojenia SMCLK a modu pocitadla
 * 6)Na zaver sme povolili prerusenia, nastavili interrupt, vynulovali flag a mohli sme pouzivat tlacidla na ovladanie LEDiek
 */
