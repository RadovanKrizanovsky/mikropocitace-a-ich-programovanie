...
//Radovan Krizanovsky
// MPP Cv. 8
// Analogovo/digitalny prevodnik - teplomer
//******************************************************************************

#include <msp430.h>
#include <stdint.h> //budeme pretipovavat na 16b a 32b

#define FS_H        P1OUT |= BIT6;
#define FS_L        P1OUT &= ~BIT6;

#define ADC_REF_VOLTAGE 1500

void fs_strobe(void);

unsigned char jed = 4, des = 3, sto = 2, tis = 1;
const unsigned char tvary_cislic[11] = { 0x3F, 6, 0x5b, 0x4f, 0x66, 0x6D, 0x7D,
                                         0x07, 0x7F, 0x6F, 0x40 };
signed int pomoc = 0;
float vysledok;
...
uint16_t adcRaw; //vyuzivame stdint.h
uint16_t voltage;
...
__interrupt void ADC10_ISR(void)
{
    ...
    voltage = (uint16_t) (((uint32_t) adcRaw * ADC_REF_VOLTAGE) >> 10); //Hlavny vypocet s right shiftom pre display

    temperature = (uint16_t) (((uint32_t) voltage - 986) * 10000 / 355);

    /************** prevod bin do BCD ******************/
    //pomoc=vysledok;
    pomoc = (int) temperature;	//vypisujeme teplotu
    //pri odcitavani moze byt "pomoc" aj zaporna
    ...
    if (tis > 9)
    {
        ...
        des = 10;
        jed = 10;
    } //cislo vacsie ako 9999
    //vypise pomlcky
    //potlacenie prvej nuly??

    /****************** vysielanie dat na cislicovky****************/

    UCB0TXBUF = ~(tvary_cislic[jed]);//kopíruj dáta do registra a start vysielania
...
__interrupt void dalsie_cislicovky(void)
{
    ...
    case 2:
        UCB0TXBUF = ~(tvary_cislic[sto] | 0x80); //pidali sme  desatinnu bodku
...
/*
 * ZAVER:
 * 1)Programovali sme v jazyku c
 * 2)Natavili sme pouzivanie teplomera
 * 3)Podla grafu a vypoctu vieme ktorej hodnote napatia patri aka hodnota v stupnoch celzia
 * 4)Upravili sme vypocet tak aby nepracoval s float-mi
 * 5)Na zaver sme do display-a zaviedli ciarku
 */
